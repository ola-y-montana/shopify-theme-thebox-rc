...
this.Shopify.theme.PredictiveSearchComponent = (function(PredictiveSearch) {
  ...
  PredictiveSearchComponent.prototype._handlePredictiveSearchSuccess = function(
    ...
    // HIDE COSTO DE ENVIO ON SEARCH
    this.results.products = _.filter(this.results.products, function(result) {
      	if (!result.vendor) return true
      	return result.vendor.toLowerCase().indexOf('oculto') === -1
    });
...
theme.MobileNav = (function() {
  ...
  function toggleMobileNav() {
    if (cache.$mobileNavToggle.hasClass(classes.mobileNavCloseIcon)) {
      ...
      if (typeof bodyScrollLock !== 'undefined' && bodyScrollLock.clearAllBodyScrollLocks) {
		bodyScrollLock.clearAllBodyScrollLocks()
	  }
    } else {
      openMobileNav();
      if (typeof bodyScrollLock !== 'undefined' && bodyScrollLock.disableBodyScroll) { 
        var $targetEl = $('.mobile-nav-wrapper')
        $targetEl.height('85vh')
		$targetEl[0] && bodyScrollLock.disableBodyScroll($targetEl[0])
	  }
    }
  }
...
theme.Slideshow = (function() {
  ...
  function slideshow(el, sectionId) {
    ...
    var sideArrows = this.$slideshow.data('side-arrows');
    ...
    if (this.autorotate && this.$arrowsMobile.length) {
      $(document).scroll(
        $.debounce(
          250,
          function() {
    ...
    this.$slideshow.slick(this.settings);
   	...
   	// Enable external arrows functiontionality
    if (sideArrows && this.$slideshow.hasClass('slick-initialized')) {
      var $prevArrow = this.$wrapper.find('.side-arrow.side-arrow-left')
      var $nextArrow = this.$wrapper.find('.side-arrow.side-arrow-right')
...
(function() {
  ...
  function isPredictiveSearchSupported() {
    var shopiyFeaturesJson = $('#shopify-features').text() || '{}';
    var shopifyFeatures = JSON.parse(shopiyFeaturesJson);
...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    _showCartPopup: function() {
      if (!this.$cartPopupWrapper.prepareTransition) {
        this.$cartPopupWrapper.prepareTransition = () => { return this.$cartPopupWrapper }
      }
      this.$cartPopupWrapper
        .prepareTransition()
...
// Fixer Header Auto Resize (Check: toggleMobileNav)
function headerSize() {
  var $headerSection = $('div#shopify-section-header');
  var $announcementBar = $('.newsletter-carousel-tbx-v2_newsletter-carousel .carousel-wrapper')
  var $counterBanner = $('.banner-html-tbx-v2_banner-counter-html-tbx-v2 .banner-x')
  if (!$headerSection || !$headerSection.length) { 
    console.warn('[The Box] headerSize(): missing header section ID');
  }
  var $headerHeight = $headerSection.outerHeight();
  if ($announcementBar.length) {
    $headerHeight += $announcementBar.outerHeight()
  }
  if ($counterBanner.length) {
    $headerHeight += ($counterBanner.outerHeight() - 1) || 0
  }
  $('#PageContainer').css('padding-top', $headerHeight);
}

// Fixer Catalog Image Resizer
function catalogImageResizer() {
    //Custom code for same heights
    var longestHeightTitle = 0;
    var biggestImage = 0
    //Reduce font-size based on lines
    $(".grid-view-item .grid-view-item__title").each(function(index){
      console.log('executed')
      var innerTitle = $(this).find(".product-card__title");
      var fontSize = parseInt(innerTitle.css("font-size"))
      var messageLines = innerTitle[0].getClientRects().length
      if(messageLines > 3){
    ...
    $(".grid-view-item .grid-view-item__image-wrapper").each(function(index){
      if($(this).height() > biggestImage){
        biggestImage = $(this).height();      
      }
    })
    //Set equal height
    $(".grid-view-item .grid-view-item__title").css({
      height: longestHeightTitle,
      display: 'block'
    }) 

    $(".grid-view-item .grid-view-item__image-wrapper").css({
      height: biggestImage,
      maxHeight: 'initial',
      paddingTop: '0px'
    }) 
    $(".grid-view-item .grid-view-item__image-wrapper div").css({
      height: biggestImage,
      paddingTop: '0px'
    }) 
    $(".grid-view-item .grid-view-item__image-wrapper img").css({
      height: biggestImage,
      objectFit: 'contain'
    }) 
}

function onbackRefresh() {
  if (performance.navigation.type === 2 && $('.template-cart').length) {
...
$(function() {
   ...
   if ($('#Collection').length && !$('.boost-pfs-filter-products').length) {
     catalogImageResizer()
   }

  $(document).on('click', '.newsletter__submit', function(event) {
    event.preventDefault()

    $acceptTermsErrorLabel = $('.accept-terms-error-label')
    $acceptTermsErrorLabel.hide()

    if (!$('#newsLetterAcceptTerms').is(':checked')) {
      $acceptTermsErrorLabel.delay(2000).show();
      return 
    }

    $(this).submit();
  });
  $(document).on('change', '#newsLetterAcceptTerms', function(event) {
    $acceptTermsErrorLabel = $('.accept-terms-error-label')
    if (this.checked) {
		$acceptTermsErrorLabel.hide();
    } else {
      $acceptTermsErrorLabel.show();
    }
  });
});
...
(function() {
  ...
  var checkoutUrl = urlParams.get('return_url') || urlParams.get('checkout_url') ;
  
  if (checkoutUrl) {
  	REDIRECT_PATH = checkoutUrl;
  }
  ...
  if ($form) {
    ...
    $('.site-footer__linklist-item a:contains("Libro de Reclamaciones")').html('<img src="https://cdn.shopify.com/s/files/1/0373/4513/2676/files/LIBRO_DE_RECLAMACIONES.jpg?v=1676929497" height="150">')
  .css('border-bottom', '0')
